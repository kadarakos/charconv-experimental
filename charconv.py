...
def convert_pytorch_outputs(model: Model, X_Ytorch: Any, is_train: bool):
    X, Ytorch = X_Ytorch
    ...
    def reverse_conversion(dY: Any) -> ArgsKwargs:
        ...
        return ArgsKwargs(args=([],), kwargs={"grad_tensors": []})
...
class PytorchCharConv(torch.nn.Module):
    ...
    def __init__(self, width, num_chars=257, char_dim=64,
        ...
        self.conv1d = torch.nn.Conv1d(char_dim, width,
...
def build_UTF8Conv(width: int) -> Model[List[Doc], List[Floats2d]]:
    ...
    model = chain(
        docs2utf8(),
        with_padutf8arrays(PyTorchWrapper(PytorchCharConv(width),
                                          convert_outputs=convert_pytorch_outputs)),
    )
    return model
